    ...
    <?php
        if(isset($_POST['account'])){
            ...
            if(emailCheck($email)==false){
                $msg[]=msgShow('Invalid E-mail Address','warning');
                $validation=false;
            }
            if(insEmail($email,['jnu.edu.bd'])==false) {
                $msg[]=msgShow('Your mail is not our Institute Mail','warning');
                $validation=false;
            } 
            if(ageCheck($age)==false){
                $msg[]=msgShow('Your age is not suitable','warning');
                $validation=false;
            } if(cellCheck($cell)==false) {
                $msg[]=msgShow('Your Cell not is Elligible','danger');
                $validation=false;
            }
            if( $validation==true) {
